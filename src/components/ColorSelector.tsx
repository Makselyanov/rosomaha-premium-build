...
export default function ColorSelector({ colors, selectedColor, onColorChange }: ColorSelectorProps) {
  ...
  const isLightColor = (hex: string) => {
    ...
    // Специальная проверка для белого цвета
    if (hex.toUpperCase() === '#FFFFFF' || hex.toUpperCase() === '#FFF') return true;
    const r = parseInt(hex.slice(1, 3), 16);
    const g = parseInt(hex.slice(3, 5), 16);
    const b = parseInt(hex.slice(5, 7), 16);
    const luminance = (0.299 * r + 0.587 * g + 0.114 * b) / 255;
    return luminance > 0.6; // Снижен порог для более надёжного определения светлых цветов
  };
