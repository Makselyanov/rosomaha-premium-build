...
case "$MODE" in
  ...
  apply|rollback|diagnose-recovery|repair-release-root)
    [[ "$BUNDLE_DIR" =~ ^/tmp/rosomaha-main-price-release-cb77869-[0-9a-f]{16}$ ]] || {
...
def validate_bundle(bundle):
    match = re.fullmatch(r"/tmp/rosomaha-main-price-release-cb77869-([0-9a-f]{16})", str(bundle))
    if not match:
...
def apply_release(bundle):
    ...
    with open_lock(create=True) as lock:
        ...
        try:
            ...
            if not release_switched or not release_path(new_release) or not re.fullmatch(r"[0-9]{8}-[0-9]{6}-prices-cb77869", Path(new_release).name):
...
def diagnostic_labels(value, exact_new):
    safe = [
        ...
        if isinstance(item, str) and re.fullmatch(r"/var/www/rosomaha/_releases/[0-9]{8}-[0-9]{6}-prices-cb77869", item)
    ]
...
def read_diagnostic_apply_receipt(bundle, baseline):
    ...
    if (
        ...
        or not re.fullmatch(r"/var/www/rosomaha/_releases/[0-9]{8}-[0-9]{6}-prices-cb77869", exact_new)
